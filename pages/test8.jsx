...
const PassportChecker = () => {
  const [passportNumber, setPassportNumber] = useState("");
  const { data, error, isLoading } = useQuery("person", () =>
    getPersonByPassport(passportNumber)
  );
  ...
  if (data?.data?.persons?.length > 0) {
    const person = data.data.persons[0];
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      />
      <button onClick={handleCheckPassport}>Check</button>
    </form>
  );
};

const queryClient = new QueryClient();
const CheckingPassport = () => {
  return (
...
export default CheckingPassport;
